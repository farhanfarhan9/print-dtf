    ...
    <div class="pt-12">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
            ...
            <div class="flex space-x-4">
                <x-input wire:model.live="startDate" type="date" placeholder="Tanggal mulai" class="w-full {{ $viewMode == 'monthly' ? 'hidden' : '' }}" />
                <x-input wire:model.live="startDate" type="month" placeholder="Bulan mulai" class="w-full {{ $viewMode == 'daily' ? 'hidden' : '' }}" />

                <x-input wire:model.live="endDate" type="date" placeholder="Tanggal akhir" class="w-full {{ $viewMode == 'monthly' ? 'hidden' : '' }}" />
                <x-input wire:model.live="endDate" type="month" placeholder="Bulan akhir" class="w-full {{ $viewMode == 'daily' ? 'hidden' : '' }}" />
            </div>
                                ...
                                <div class="p-5 mb-5 border rounded-md">
                                    <table
                                        ...
                                        <tbody>
                                            ...
                                            @endforelse
                                            <tr wire:key="key-{{ $key }}"" class="bg-green-600 text-white">
                                                <th class="px-6 py-2 text-medium font-bold text-center rounded-s-lg whitespace-nowrap">
                                                    Total
                                                </th>
                                                <th class="px-6 py-2 text-medium font-bold whitespace-nowrap">
                                                    {{ count($dailyPurchase) }} Customer
                                                </th>
                                                <th colspan="2" class="px-6 py-2 text-medium font-bold rounded-e-lg whitespace-nowrap">
    ...
    @elseif($viewMode == 'monthly')
        ...
        <div class="pt-12">
            @forelse ($monthlyGroupPurchases as $purchase_month => $monthlyPurchase)
                <div class="mb-10" wire:key='key-{{ \Carbon\Carbon::createFromFormat('Y-m', $purchase_month)->isoFormat('MM-YYYY') }}'>
                    <x-card>
                        @php
